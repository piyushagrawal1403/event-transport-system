...
export default function RequestRide() {
  ...
  useEffect(() => {
    ...
    getGuestRides(guestPhone).then(res => {
      if (res.data.length > 0) navigate('/status');
    }).catch(() => {});
  ...
  const isOthers = selectedLocation?.name === 'Others';

  const handleSubmit = async () => {
    if (!selectedLocationId || !venue) return;

    if (isOthers && !customDestination.trim()) {
    ...
    try {
      ...
      while (remaining > 0) {
        ...
        await createRide({
          ...
          ...(isOthers ? { customDestination: customDestination.trim() } : {}),
        });
        remaining -= count;
      }
      navigate('/status');
        ...
        <div className="max-w-md mx-auto p-4 space-y-4">
          ...
          <div className="bg-white rounded-xl shadow-sm p-4 space-y-3">
            <label className="flex items-center gap-2 text-sm font-medium text-gray-700">
              ...
              {direction === 'TO_VENUE' ? 'Pickup Location' : 'Dropoff Location'}
            </label>
            <select
                value={selectedLocationId ?? ''}
                onChange={(e) => {
                  setSelectedLocationId(Number(e.target.value));
                  setCustomDestination(''); // reset when switching
                  setError('');
                }}
            ...
            {/* "Others" — custom destination input */}
            {isOthers && (
                <div className="space-y-1.5">
                  ...
                  <input
                      ...
                      onChange={e => { setCustomDestination(e.target.value); setError(''); }}
                      placeholder="e.g. MG Road, Indiranagar Metro Station…"
                      className="w-full py-3 px-4 border-2 border-amber-300 rounded-xl bg-amber-50 focus:ring-2 focus:ring-amber-400 focus:border-amber-400 outline-none text-sm placeholder-amber-400"
                  />
                  <p className="text-xs text-amber-600">
                    This destination will be shown to the driver and the admin.
                  </p>
                </div>
            )}
          ...
          <button
              ...
              disabled={loading || !selectedLocationId || (isOthers && !customDestination.trim())}
